...
def signal_simple_bolling2(df, para):
    """
    # 布林线策略
    # 布林线中轨：n天收盘价的移动平均线
    # 布林线上轨：n天收盘价的移动平均线 + m * n天收盘价的标准差
    ...
    # !计算均线偏移
    df['upjun'] = df['median'] + df['median'] * junptc  #上均线
    df['downjun'] = df['median'] - df['median'] * junptc  #下均线

    # !找出穿透均线信号
    condition1 = df['close'] < df['upjun']
    condition2 = df['close'].shift(1) >= df['upjun'].shift(1)
    df.loc[condition1 & condition2, 'signal_long2'] = 1

    # !找出穿透均线信号
    condition1 = df['close'] > df['downjun']
    condition2 = df['close'].shift(1) <= df['downjun'].shift(1)
    df.loc[condition1 & condition2, 'signal_short2'] = -1

    # ===计算信号
    # 找出做多信号
    condition1 = df['close'] > df['upper']  # 当前K线的收盘价 > 上轨
    condition2 = df['close'].shift(1) <= df['upper'].shift(1)  # 之前K线的收盘价 <= 上轨
    df.loc[condition1 & condition2, 'signal_long'] = 2  # 将产生做多信号的那根K线的signal设置为1，1代表做多
    ...
    condition1 = df['close'] < df['lower']  # 当前K线的收盘价 < 下轨
    condition2 = df['close'].shift(1) >= df['lower'].shift(1)  # 之前K线的收盘价 >= 下轨
    df.loc[condition1 & condition2, 'signal_short'] = -2  # 将产生做空信号的那根K线的signal设置为-1，-1代表做空
    ...
    # !合并信号均线偏移信号和做多信号
    df['temp_signal'] = df[['signal_long', 'signal_long2']].sum(axis=1, min_count=1, skipna=True)
    temp = df[df['temp_signal'].notnull()][['temp_signal']]  # 1,0,22
    temp = temp[temp['temp_signal'] != temp['temp_signal'].shift(1)]  # 1,0,2
    temp = temp[(temp['temp_signal'] == 1) & (temp['temp_signal'].shift(1) == 2) | (temp['temp_signal'] == 0)]
    temp = temp[temp['temp_signal'] != temp['temp_signal'].shift(1)]  # 1,0,2
    df['signal_long'] = temp['temp_signal']

    # !合并信号均线偏移信号和做空信号
    df['temp_signal'] = df[['signal_short', 'signal_short2']].sum(axis=1, min_count=1, skipna=True)
    temp = df[df['temp_signal'].notnull()][['temp_signal']]  # 1,0,22
    temp = temp[temp['temp_signal'] != temp['temp_signal'].shift(1)]  # 1,0,2
    temp = temp[(temp['temp_signal'] == -1) & (temp['temp_signal'].shift(1) == -2) | (temp['temp_signal'] == 0)]
    temp = temp[temp['temp_signal'] != temp['temp_signal'].shift(1)]  # 1,0,2
    df['signal_short'] = temp['temp_signal']
    ...
    # ===删除无关变量
    df.drop(['median', 'std', 'upper', 'lower', 'signal_long', 'signal_short', 'temp_signal'], axis=1, inplace=True)

    return df

def signal_simple_bolling_para_list2(m_list=range(50, 100, 10), n_list=[i / 10 for i in list(np.arange(2, 5, 1))],junptc_list=[i / 100 for i in list(np.arange(3, 5, 1))]):
    """
    产生布林 策略的参数范围
    :param m_list:
    :param n_list:
